...
在 FlClash 0.8.98 上为 ChatGPT 和 Claude 给节点打分：不调用模型，不下载大文件，只看节点能不能用、到源站的往返有多短、空闲连接能撑多久。
...
代理页顶部常驻显示当前选择的订阅和节点；所有订阅分组默认折叠，点击组头展开或收起。展开后是紧凑单行小格，显示累计得分和估算往返，悬停可看 PING、机房到源站、401 实测、出口机房、淘汰赛名次和最新失败原因。各订阅内按累计得分降序，同分按最近一次估算往返升序，未测速节点置后。

## 测速流程

1. **初筛**（约 15–30 秒）：每个节点同时连 `chatgpt.com` 和 `api.anthropic.com`，只认未登录的 401；403（地区封锁或 Cloudflare 拦截）标为拒绝，两边任一不通都不参加后续比赛。ChatGPT 连接上顺序交替发 HTTP/2 PING 和 401 请求（6 次 PING、3 次请求），出口机房取自 `Cf-Ray`。32 路并发，30 秒截止。
2. **估算往返**：PING 由 Cloudflare 机房直接应答，只反映节点自己那段路，重测波动约 3–4 ms；机房到 ChatGPT 源站那段每次请求波动约 100 ms，但同一机房的节点共享。所以估算往返 = 节点 PING 中位数 + 该机房各节点「401 中位数 − PING 中位数」的中位数。至少 3 个节点的机房才有这个统计并参加排名，更小的机房节点直接用自己的 401 中位数。
3. **机房统计**：机房往返取其节点估算往返的中位数，选出最快的两个机房。
4. **淘汰赛**（通常 1–3 分钟，最长 15 分钟）：两个机房的所有节点保留初筛那条 ChatGPT 连接，统一发一次请求后进入空闲。中继空闲超时会用 FIN/RST 关闭连接，被动记录谁先断，不发任何会重置空闲计时的数据。先断的先淘汰，直到剩最后一个。5 秒内先后断掉视为平手，按估算往返排先后；最后一名幸存者要比上一个断线者多撑 5 秒才算独胜。平手里估算往返与档首相差不到 15 ms 的并列同一名次。
5. **计分**：前四个名次位依次值 4、3、2、1 分，并列者平分所占名次位的分数（例如 6 个并列第一平分 10 分，各 1.67 分），按节点配置指纹累计写入 `node-score-v1.json`。配置指纹改变视为新节点，不继承旧分。最近一次失败会覆盖延迟显示，但不扣分。
